...
execute in the_end at @a[gamemode=!spectator] as @e[type=minecraft:enderman] run execute as @s at @s unless block ~ ~-1 ~ #december:city_blocks unless entity @e[type=shulker,distance=0..5] run tag @s add NoShulker
